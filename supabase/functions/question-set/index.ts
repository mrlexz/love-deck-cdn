...
serve(async (req) => {
  ...
  try {
    ...
    const questionSetId = url.searchParams.get("id");
    ...
    if (req.method === "GET") {
      ...
      if (questionSetId) {
        query = query.eq("id", questionSetId).single();
      }
      ...
      return new Response(
        JSON.stringify({
          success: true,
          data: data,
          message: questionSetId
            ? "Question set retrieved successfully"
            : "Question sets retrieved successfully",
        }),
        {
    ...
    if (req.method === "PUT") {
      ...
      if (updateError) {
        console.error("Question Set update error:", updateError);
        throw updateError;
      }
